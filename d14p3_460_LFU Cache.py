...
class LFUCache:
    ...
    def put(self, key: int, value: int) -> None:
        ...
        self.update_count(key)
        self.min_count = 1
        # min(self.min_count, self.key_count_cache[key])
...
if __name__ == "__main__":
    test_cases = [
        {"input": [["LFUCache","put","put","put","put","get"], [[2],[3,1],[2,1],[2,2],[4,4],[2]]], "expected_output": [None,None,None,None,None,2]}
    ]
    for test_case in test_cases:
        ...
        solution = None  # 初始化 LFUCache 的變數
        
        for action, param in zip(actions, params):
            if action == "LFUCache":
                solution = LFUCache(*param)  # 初始化 LFUCache
                result.append(None)
            elif action == "put":
